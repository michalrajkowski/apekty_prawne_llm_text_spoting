"""Specialized unit tests for the SeqXGPT detector adapter."""
...
def _install_fake_runtime(monkeypatch: pytest.MonkeyPatch, cuda_available: bool = True) -> FakeTorch:
    ...
    monkeypatch.setattr(
        "apm.detectors.adapters.seqxgpt.importlib.import_module",
        fake_import_module,
    )
    return fake_torch
...
def test_list_supported_variants_filters_above_vram_limit() -> None:
    variant_ids = SeqXGPTDetector.list_supported_variants(config={"max_supported_vram_gb": 6.0})

    assert "gpt2_small" in variant_ids
    assert "gpt2_medium" in variant_ids
    assert "gpt2_large" in variant_ids
    assert "gpt_j_6b" not in variant_ids
