"""Specialized unit tests for the Fast-DetectGPT detector adapter."""
...
class FakeModel:
    ...
    def __call__(self, **batch: Any) -> Any:
        ...
        if self._model_id == "openai-community/gpt2":
            loss_value = 0.3 if is_ai else 1.3
        else:
            loss_value = 0.8 if is_ai else 1.1
...
def _install_fake_runtime(monkeypatch: pytest.MonkeyPatch, cuda_available: bool = True) -> FakeTorch:
    ...
    monkeypatch.setattr(
        "apm.detectors.adapters.fast_detectgpt.importlib.import_module",
        fake_import_module,
    )
    return fake_torch
...
def test_predict_batch_returns_scores_for_each_text(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    detector = FastDetectGptDetector.initialize(config={"device": "cuda"})
