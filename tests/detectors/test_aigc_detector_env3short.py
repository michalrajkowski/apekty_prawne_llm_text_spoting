"""Specialized unit tests for the AIGC Detector V3-Short adapter."""
...
class FakeModel:
    ...
    def __call__(self, **batch: Any) -> Any:
        ...
        for text in batch["__texts__"]:
            lowered = text.lower()
            if "ai" in lowered:
                logits.append([0.25, 2.25])
            else:
                logits.append([2.0, 0.1])
...
def _install_fake_runtime(monkeypatch: pytest.MonkeyPatch, cuda_available: bool = True) -> FakeTorch:
    ...
    monkeypatch.setattr(
        "apm.detectors.adapters.aigc_detector_env3short.importlib.import_module",
        fake_import_module,
    )
    return fake_torch
...
def test_predict_batch_returns_scores_for_each_text(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    detector = AIGCDetectorEnv3Short.initialize(config={"device": "cuda", "ai_label_index": 1})
...
def test_delete_releases_cuda_cache_when_cuda_selected(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    detector = AIGCDetectorEnv3Short.initialize(config={"device": "cuda"})
