"""Materialize GriD split outputs with deterministic sampling."""
...
from apm.data.adapters.grid_adapter import GridAdapter
...
def materialize_grid_samples(
    ...
    """Load GriD splits and persist sampled normalized outputs."""

    adapter = GridAdapter.from_config_path(
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Materialize GriD sampled split outputs.")
    parser.add_argument("--project-root", type=Path, default=Path.cwd(), help="Repository root path.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/datasets/grid.dataset.json"),
        help="Path to GriD dataset config JSON.",
    )
