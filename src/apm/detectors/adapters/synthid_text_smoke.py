"""Smoke-run entry point for SynthID Text detector scoring."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run smoke inference for SynthID Text detector.")
    parser.add_argument("--project-root", type=Path, default=Path.cwd(), help="Repository root path.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/detectors/synthid_text.detector.json"),
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("runs/detectors/synthid_text_smoke.jsonl"),
...
def _score_variant(config: dict[str, Any], texts: list[str], variant_id: str) -> list[dict[str, Any]]:
    ...
    detector = SynthIDTextDetector.initialize(config=variant_config)
    try:
...
def main() -> int:
    ...
    if args.all_variants:
        variant_ids = SynthIDTextDetector.list_supported_variants(config=config)
    else:
        variant_ids = [str(config.get("variant_id", "gpt2"))]
