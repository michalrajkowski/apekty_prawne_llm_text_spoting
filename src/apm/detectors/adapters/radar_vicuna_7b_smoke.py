"""Smoke-run entry point for RADAR-Vicuna-7B detector scoring."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run smoke inference for RADAR-Vicuna-7B detector.")
    parser.add_argument("--project-root", type=Path, default=Path.cwd(), help="Repository root path.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/detectors/radar_vicuna_7b.detector.json"),
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("runs/detectors/radar_vicuna_7b_smoke.jsonl"),
...
def main() -> int:
    ...
    detector = RadarVicuna7BDetector.initialize(config=config)
    try:
