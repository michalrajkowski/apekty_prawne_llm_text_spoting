"""Smoke-run entry point for Ghostbuster detector scoring and HC3 validation."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run smoke inference for Ghostbuster detector.")
    parser.add_argument("--project-root", type=Path, default=Path.cwd(), help="Repository root path.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/detectors/ghostbuster.detector.json"),
        help="Path to detector config JSON.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("runs/detectors/ghostbuster_smoke.json"),
...
def main() -> int:
    ...
    detector = GhostbusterDetector.initialize(config=config)
    ...
    if separation_summary["passed"] is not True:
        raise ValueError(
            "HC3 separation check failed for Ghostbuster: "
