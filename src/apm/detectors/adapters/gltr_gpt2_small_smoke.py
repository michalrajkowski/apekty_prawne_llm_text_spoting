"""Smoke-run entry point for GLTR GPT-2 small detector scoring."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run smoke inference for GLTR GPT-2 small detector.")
    parser.add_argument("--project-root", type=Path, default=Path.cwd(), help="Repository root path.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/detectors/gltr_gpt2_small.detector.json"),
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("runs/detectors/gltr_gpt2_small_smoke.jsonl"),
...
def main() -> int:
    ...
    detector = GLTRGpt2SmallDetector.initialize(config=config)
    try:
