"""Smoke-run entry point for AIGC Detector V3 scoring."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run smoke inference for AIGC Detector V3.")
    parser.add_argument("--project-root", type=Path, default=Path.cwd(), help="Repository root path.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/detectors/aigc_detector_env3.detector.json"),
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("runs/detectors/aigc_detector_env3_smoke.jsonl"),
...
def main() -> int:
    ...
    detector = AIGCDetectorEnv3.initialize(config=config)
    try:
